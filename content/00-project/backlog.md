...
| RES-003-pricing | Ручная сверка прайса YC FM в браузере (`https://yandex.cloud/ru/docs/foundation-models/pricing` — за CAPTCHA). Цена за 1k input-токенов для `text-search-doc` и `text-search-query` — копия в чат | — | секция «Цена» в [yc-foundation-models.md](../10-domain/research/yc-foundation-models.md) | todo | Phase 0 / iter 1 |
| RES-003 | Бюджет YC FM: расчёт стоимости full-rescan на медиане 100k–300k объектов. Rough estimate в yc-foundation-models.md (4-20 тыс. ₽/тенант). Финал — после RES-003-pricing | RES-003-pricing | `content/00-project/budget-estimate.md` | blocked | Phase 0 / iter 1 |
| OWNER-003 | Подтвердить single vs multi-tenant на стенде `llm2` (OQ-UC1-1, OA-3). Если multi-tenant — открывать дополнительный ADR на тенант-изоляцию | OWNER-001 | апдейт в [ADR-004](adr/004-vector-storage-schema.md), пометка решения в issue | todo | Phase 0 |
| OWNER-004 | Решить auth-режим YC: IAM-token через JWT (production-ready) или статический Api-Key (PoC-shortcut). Решение фиксируется в новом `ADR-yc-auth` или в апдейте ADR-009 | OWNER-002 | новый `ADR-yc-auth` | todo | Phase 0 / iter 1 |
