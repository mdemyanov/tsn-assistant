...
| **RES-009.1** | ✅ Закрыт 2026-05-01. Domain & data analysis на корпусе llm2: 6 259 issue / 32 298 comment / 113 KB-статей с `content`. Ключевые числа: KB.content p95 = 26 014 ток. (chunking обязателен для 47,8 %); issue.composite_extended p95 = 209 ток. (single-vector работает на 99,6 %); PoC-whitelist (subject+cancelReason) median 22 ток. — недостаточен; problem не валидируется (3 объекта); ground truth UC3 = 2,4 %. Рекомендация для волны 2: **стратегия D (hybrid)** — issue/A + KB/C | OWNER-001, OWNER-002, RES-002-closeout | [vector-storage-domain.md](../10-domain/research/vector-storage-domain.md) + [length-distribution.csv](../10-domain/research/raw/length-distribution.csv) | done | Phase 0 / iter 1 |
| **RES-009.2** | Storage strategies comparison: сравнение A (single-vector) / B (multi-vector by content-type) / C (chunked+parent-rollup) / D (hybrid two-tier) / E (per-related-vector) по 8 dimensions с учётом стат из RES-009.1. Search aggregation алгоритмы (max-pool, RRF, parent-rollup). ACL-инвариант (только KB через kbAccesses). Связанные служебные таблицы (model_version, whitelist_version, audit, chunk_meta) — что выносить, что inline. Решить OQ-1..OQ-10 из RES-009.1 §4.4. Smoke YC FM с длинным текстом (3 000 ток.) для верификации лимита OQ-8 | RES-009.1 | `content/10-domain/research/vector-storage-strategies.md` | ready | Phase 0 / iter 1 |
| **RES-009.3** | API-контракт модуля + DDL-скелет + поэтапная рекомендация + реестр изменений в ADR-001..010. Минимум 3 варианта API: `vectorize(compositeText)` vs `vectorize(subject, description, related[])` vs `vectorize(payload tree)`. DDL для рекомендованной стратегии. Поэтапная рекомендация PoC iter 1→4→MVP | RES-009.2 | `content/10-domain/research/vector-storage-strategy.md` (финал) | blocked | Phase 0 / iter 1 |
