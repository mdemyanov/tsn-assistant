...
Превратить мульти-профильный шаблон `project_template` в **моно-целевой шаблон для управления товариществом собственников недвижимости в широком смысле**: ТСЖ (МКД), СНТ/ОНТ (садовые/огороднические), ЖСК (жилищно-строительные кооперативы).

**Картина успеха.** Председатель товарищества (МКД или СНТ) клонирует репозиторий → выполняет `/init` (отвечает на ~8 вопросов о товариществе, первый из которых — тип организации) → получает рабочий Gramax-каталог со структурой по 10 разделам, набор из 8 агентов и ~17 slash-команд, покрывающих весь цикл управления: документооборот, юр.анализ, финансовый анализ, общие собрания, коммуникации с членами товарищества, договорная работа.

**Универсальность по типам:** агенты `legal`, `finance`, `docs`, `comms` адаптируют ответы под тип организации (ЖК РФ для МКД vs ФЗ-217 для СНТ/ОНТ), беря тип из `content/01-property/passport.md`. Терминология в дефолтных шаблонах нейтральная ("собственники", "общее имущество", "общее собрание"); специфика (квартиры/участки, РСО/охрана) подгружается из passport.

**Референсы:**
- Реальный vault: `/Users/mdemyanov/Documents/TSN16k2/` (Obsidian, ТСН «Лайф 2», МКД, Москва) — основная база структуры и ролей
- Best practices ТСН/ТСЖ/СНТ/ЖСК (ЖК РФ, ФЗ-217 от 29.07.2017, региональные НПА)
...
| `legal` | subagent | Sonnet | Юр.анализ договоров, претензии, разъяснение НПА. Двухветочная база: МКД (ЖК РФ, ГК РФ) и СНТ/ОНТ (ФЗ-217 от 29.07.2017, ГК РФ). Тип организации читает из passport. | `legal_analyst` |
| `finance` | subagent | Sonnet | Тарифы (МКД) или членские/целевые взносы (СНТ), бюджеты, сметы, биллинг РСО / расчёт по участкам. Тип организации учитывает. | `financial_analyst` |
...
│
├── 01-property/               # Объект управления (дом или территория с участками)
│   ├── _index.md
│   ├── passport.md            # ★ заполняется на /init: тип организации, адрес, площадь, год
│   ├── premises/              # помещения/участки
│   │   ├── _index.md
│   │   ├── apartments.md      # для МКД/ЖСК
│   │   ├── commercial.md      # для МКД
│   │   └── plots.md           # для СНТ/ОНТ (участки)
│   └── equipment/             # инженерное оборудование / общая инфраструктура
│       └── _index.md          # для МКД — лифты, домофон, ИТП; для СНТ — скважина, эл.сети, дороги, КПП
│
├── 02-owners/                 # Собственники / члены товарищества
│   ├── _index.md
...
│       └── _index.md
│
├── 04-general-meeting/        # Общее собрание (ОСС для МКД / ОС для СНТ)
│   ├── _index.md
│   ├── procedures.md          # порядок проведения (зависит от типа: ЖК РФ vs ФЗ-217)
...
│   ├── uk/                    # управляющие компании (МКД)
│   │   └── _index.md
│   ├── rso/                   # ресурсоснабжающие организации (МКД; СНТ — реже)
│   │   └── _index.md
│   └── service/               # обслуживание (МКД: лифты/уборка; СНТ: вывоз мусора, охрана, скважина)
│       └── _index.md
│
...
│   ├── _index.md
│   └── authorities.md         # органы власти (МКД: ГЖИ, прокуратура, мэрия; СНТ: Росреестр, Россельхознадзор, муниципалитет)
│
└── 10-archive/                # Архив
    └── _index.md
```
...
4. Подстановка плейсхолдеров в:
   ...
   - `content/_index.md`
   - `content/01-property/passport.md` (адрес)
...
По одному вопросу. На skip — TODO-маркер остаётся в файле. Первый вопрос — тип организации — определяет, какие НПА цитируют агенты и какую терминологию использовать.

| # | Тема | Вопрос | Куда пишем |
|---|------|--------|------------|
| 1 | Тип организации | "Тип товарищества: МКД (ТСЖ/ЖСК) / СНТ / ОНТ / другое?" | `CLAUDE.md` (контекст), `01-property/passport.md` |
| 2 | Регион | "Регион/субъект РФ (для НПА): Москва / СПб / Московская область / другой" | `CLAUDE.md`, `09-contacts/authorities.md` |
| 3 | Объект | "Сколько объектов: для МКД — кол-во квартир + коммерческих; для СНТ — кол-во участков" | `01-property/passport.md`, `01-property/premises/` |
| 4 | Площадь | "Общая площадь (м² для МКД; га для СНТ)" | `01-property/passport.md` |
| 5 | Год создания | "Год постройки дома / год образования товарищества" | `01-property/passport.md` |
| 6 | Состав правления | "Перечисли членов правления (ФИО + роль + контакт). Можно по одному." | `03-board/actors.md` |
| 7 | Подрядчики | "Ключевые обслуживающие организации (МКД: УК, РСО; СНТ: вывоз мусора, охрана, эл.сети). Опц." | `06-contracts/registry.md` |
| 8 | Особенности | "Кратко: специфика товарищества (опц.) — судебные споры, крупные проекты, особенности". | `CLAUDE.md` (Project-specific) |
...
- Новый: `scripts/test-init-tsn.sh` — smoke-test полного init flow с фиктивными ТСН-параметрами:
  ...
  - проверить, что `content/01-property/passport.md` создан и заполнен
...
- `content/01-property/` ... `content/10-archive/` — структура согласно §2.4
...
- `content/` — Gramax-каталог с 10 разделами (объект, собственники, правление, общее собрание, финансы, договоры, юр., проекты, контакты, архив)
...
- [ ] `/init` Phase 2 заполняет `01-property/passport.md`, `03-board/actors.md` по ответам пользователя
