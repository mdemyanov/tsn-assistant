...
| **RES-009.1** | Domain & data analysis: бизнес-кейсы UC1/UC2/UC3 в терминах payload'а от caller'а; реестр связанных FQN на `llm2`; распределение длин текста (issue.description, issueComment.text, problem.description, kbArticle.text) и комментариев на заявку — HQL-скрипт через `smps`. **Scope:** модуль отвечает только за хранение/векторизацию/поиск; триггеры и отбор контента — out of scope (внешний инструмент). KB-section context, comment filtering, re-vectorization triggers — исключены | OWNER-001, OWNER-002, RES-002-closeout | `content/10-domain/research/vector-storage-domain.md` + `content/10-domain/research/raw/length-distribution.csv` | todo | Phase 0 / iter 1 |
| **RES-009.2** | Storage strategies comparison: сравнение A (single-vector) / B (multi-vector by content-type) / C (chunked+parent-rollup) / D (hybrid two-tier) / E (per-related-vector) по 8 dimensions с учётом стат из RES-009.1. Search aggregation алгоритмы (max-pool, RRF, parent-rollup). ACL-инвариант (issue.internalComments). Связанные служебные таблицы (model_version, whitelist_version, audit, chunk_meta) — что выносить, что inline | RES-009.1 | `content/10-domain/research/vector-storage-strategies.md` | blocked | Phase 0 / iter 1 |
